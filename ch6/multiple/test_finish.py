...
@pytest.mark.parametrize("start_state", ["todo", pytest.param("in prog", marks=pytest.mark.smoke), "done"])
...
@pytest.fixture(params=["todo", pytest.param("in prog", marks=pytest.mark.smoke), "done"])
def start_state_fixture(request):
    return request.param


def test_finish_fixture(cards_db, start_state_fixture):
...
def test_finish_non_existence(cards_db):
    i = 123
